...
async function conectar() {
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      const conteudo = msg.message?.conversation || msg.message?.extendedTextMessage?.text || msg.message?.imageMessage?.caption || "[mídia]"
...
app.post("/request-pairing", async (req, res) => {
  ...
  if (!sock || connected) return res.json({ error: connected ? "Já conectado" : "Servidor não iniciado" })
  try {
...
<div id="pair-s"><div id="pc"><div class="st wait">Digite seu número com DDD</div><input type="tel" id="ph" placeholder="11999999999" maxlength="13"><button class="btn" onclick="reqCode()">🔑 Gerar código</button></div>
<div id="pr" style="display:none"><div class="st wait" id="ps">Gerando...</div><div class="code-box" id="pb" style="display:none"></div><div class="note" id="pi" style="display:none"><b>WhatsApp → Dispositivos vinculados → Vincular com número de telefone</b> → digite o código acima</div></div></div>
</div><script>
const s=io();
...
function reqCode(){const p=document.getElementById('ph').value.replace(/\\D/g,'');if(p.length<10){alert('Número inválido');return;}document.getElementById('pc').style.display='none';document.getElementById('pr').style.display='block';fetch('/request-pairing',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone:p})}).then(r=>r.json()).then(d=>{if(d.error){document.getElementById('ps').className='st err';document.getElementById('ps').textContent='Erro: '+d.error;}});}
...
<div class="layout">
  <div class="sidebar">
    ...
    <div class="convs" id="convs-list">
      <div style="padding:16px;text-align:center;color:rgba(148,185,210,.50);font-size:12px;">Carregando...</div>
    </div>
    <div class="sb-new">
  ...
  <div class="chat-main">
    <div class="chat-header">
      ...
      <div>
        <div class="ch-name" id="ch-name">Selecione uma conversa</div>
...
async function checkStatus() {
  try {
    ...
    const l = document.getElementById('qr-link')
...
async function carregarConvs() {
  const { data } = await sb.from('whatsapp_conversas').select('*').order('ultima_atualizacao', { ascending: false })
  todasConvs = data || []
  renderConvs(todasConvs)
}
function filtrarConvs() {
  const q = document.getElementById('search-input').value.toLowerCase()
  renderConvs(q ? todasConvs.filter(c => (c.nome||c.telefone||'').toLowerCase().includes(q)) : todasConvs)
}
async function renderConvs(lista) {
  const el = document.getElementById('convs-list')
  ...
  // Para cada conversa, busca venda
  let html = ''
  for (const c of lista) {
    const tel = c.telefone || ''
    const nome = c.nome || tel
    const prev = (c.ultimo_msg||'').substring(0,28)
    const nr = c.nao_lidas > 0 ? \`<span class="nr">\${c.nao_lidas}</span>\` : ''
    const ini = nome.split(' ').slice(0,2).map(p=>p[0]?.toUpperCase()||'').join('')
    const ativo = tel === telAtivo ? 'active' : ''
    // busca venda
    const { data: vd } = await sb.from('vendas').select('status,valor').eq('telefone', tel.replace(/\\D/g,'')).order('id',{ascending:false}).limit(1)
    let tag = ''
    if (vd && vd[0]) {
      const s = vd[0].status; const v = formatMoney(vd[0].valor)
    ...
    html += \`<div class="conv \${ativo}" onclick="selecionarConv('\${tel}','\${nome.replace(/'/g,"\\\\'")}')">
      <div class="av">\${ini}</div>
      <div class="conv-info">
        <div class="conv-top"><span class="conv-name">\${nome}</span>\${nr}</div>
...
async function selecionarConv(tel, nome) {
  ...
  document.getElementById('ch-av').textContent = nome.split(' ').slice(0,2).map(p=>p[0]?.toUpperCase()||'').join('')
...
async function carregarMsgs(tel) {
  const { data } = await sb.from('whatsapp_mensagens').select('*').eq('telefone',tel).order('timestamp').limit(100)
  const el = document.getElementById('msgs-area')
  document.getElementById('empty-msg').style.display = 'none'
  const wm = el.querySelector('.wm')
  let html = wm ? wm.outerHTML : ''
  if (!data || !data.length) { html += '<div class="empty-chat"><svg viewBox="0 0 24 24"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>Nenhuma mensagem ainda.</div>' }
  else { for (const m of data) { const cls = m.de_mim?'msg-m':'msg-d'; const ts = (m.timestamp||'').substring(0,16); html += \`<div class="\${cls}">\${m.conteudo}<div class="msg-ts">\${m.de_mim?'✓✓ ':''}\${ts}</div></div>\` } }
  el.innerHTML = html
  el.scrollTop = el.scrollHeight
}
async function carregarVenda(tel) {
  const t = tel.replace(/\\D/g,'')
  const { data } = await sb.from('vendas').select('*').eq('telefone',t).order('id',{ascending:false}).limit(1)
  vendaAtiva = data && data[0] ? data[0] : null
  renderProposta()
}
async function iniciarConv() {
  const t = document.getElementById('new-tel').value.replace(/\\D/g,'')
  if (t.length < 10) { alert('Número inválido'); return }
...
async function enviar() {
  ...
  const r = await fetch('/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to:telAtivo,message:txt})})
...
async function renderRespostas() {
  const { data } = await sb.from('whatsapp_respostas_rapidas').select('*').order('id')
  const el = document.getElementById('tab-rr')
  let html = '<div class="rp-sec">Respostas rápidas</div>'
  for (const r of (data||[])) {
    html += \`<div class="rr-item" onclick="usarResposta('\${r.texto.replace(/'/g,"\\\\'")}')">
      <div><div class="rr-title">\${r.titulo}</div><div class="rr-prev">\${r.texto}</div></div>
      <span class="rr-del" onclick="event.stopPropagation();deletarResposta(\${r.id})">✕</span>
    </div>\`
  }
...
async function novaResposta() {
  ...
  await sb.from('whatsapp_respostas_rapidas').insert({titulo:tit,texto:txt})
  renderRespostas()
}
...
function renderProposta() {
  const el = document.getElementById('tab-prop')
  if (!telAtivo) { el.innerHTML = '<div class="no-prop">Selecione uma conversa</div>'; return }
  if (!editando) {
    if (vendaAtiva) {
      el.innerHTML = \`<div class="prop-card">
        <div class="pc-header"><span class="pc-title">📄 PROPOSTA ATIVA</span><span class="pc-sync">🔄 Supabase</span></div>
        <div class="pf-label">Cliente</div><div class="pf-value">\${vendaAtiva.cliente||'—'}</div>
        <div class="pf-label">Tabela/Banco</div><div class="pf-value">\${vendaAtiva.tabela_banco||'—'}</div>
        <div class="pf-label">Valor</div><div class="pf-valor">\${formatMoney(vendaAtiva.valor)}</div>
        <div class="pf-label" style="margin-top:8px;">Status</div>
        <div style="margin-top:3px;">\${pillHtml(vendaAtiva.status||'Pendente')}</div>
        \${vendaAtiva.observacao?'<div class="pf-label" style="margin-top:6px;">Observação</div><div style="font-size:11px;color:#94a3b8;">\${vendaAtiva.observacao}</div>':''}
        <button class="edit-btn" onclick="abrirEdicao()">✏️ Editar proposta</button>
      </div>\`
    } else {
      el.innerHTML = \`<div class="no-prop">📋<br>Nenhuma proposta ainda.</div>
        <hr>
        <div class="rp-sec">+ Cadastrar venda</div>
        <div class="ef-label">Cliente</div><input class="ef-input" id="nc-cli" placeholder="Nome">
        <div class="ef-label">CPF</div><input class="ef-input" id="nc-cpf" placeholder="00000000000">
        <div class="ef-label">Tabela/Banco</div><input class="ef-input" id="nc-tab" placeholder="3RN CAPITAL...">
        <div class="ef-label">Valor R$</div><input class="ef-input" id="nc-val" placeholder="5.000,00">
        <div class="ef-label">Status</div>
        <div class="st-grid">\${STATUS_LIST.map((s,i)=>\`<div class="st-chip \${s.toLowerCase().replace(/ /g,'-').replace('ã','a').replace('ç','c')} \${i===0?'sel':''} sc-\${['pend','agpag','agass','pago','canc'][i]}" onclick="selSt(this)">\${['<span class=\\'st-dot\\'></span>','<span class=\\'st-dot\\'></span>','<span class=\\'st-dot\\'></span>','<span class=\\'st-dot\\'></span>','<span class=\\'st-dot\\'></span>'][i]}\${s}</div>\`).join('')}</div>
        <button class="save-btn" onclick="salvarNovaVenda()">💾 Salvar venda</button>\`
    }
  } else {
    const v = vendaAtiva
    el.innerHTML = \`<div class="rp-sec">✏️ Editar proposta</div>
      <div class="ef-label">Cliente</div><input class="ef-input" id="e-cli" value="\${v.cliente||''}">
      <div class="ef-label">Tabela/Banco</div><input class="ef-input" id="e-tab" value="\${v.tabela_banco||''}">
      <div class="ef-label">Valor R$</div><input class="ef-input" id="e-val" value="\${parseFloat(v.valor||0).toFixed(2).replace('.',',')}">
      <div class="ef-label">Status</div>
      <div class="st-grid">\${STATUS_LIST.map((s,i)=>{const c='sc-'+['pend','agpag','agass','pago','canc'][i];return\`<div class="st-chip \${c} \${s===v.status?'sel':''}" onclick="selSt(this)"><span class="st-dot"></span>\${s}</div>\`}).join('')}</div>
      <div class="ef-label">Observação</div><input class="ef-input" id="e-obs" value="\${v.observacao||''}">
      <div id="sync-ok" class="sync-msg">✅ Atualizado no Painel de Vendas!</div>
      <button class="save-btn" onclick="salvarEdicao()">💾 Salvar → Atualiza Painel</button>
      <div class="back-link" onclick="editando=false;renderProposta()">← voltar</div>
      <div style="background:rgba(56,189,248,.07);border:1px solid rgba(56,189,248,.18);border-radius:7px;padding:6px 9px;font-size:10px;color:#7dd3fc;text-align:center;margin-top:6px;">💡 Salvar aqui atualiza o Painel de Vendas automaticamente</div>\`
  }
}
...
async function salvarNovaVenda() {
  const cli=document.getElementById('nc-cli').value; const cpf=document.getElementById('nc-cpf').value.replace(/\\D/g,'')
  const tab=document.getElementById('nc-tab').value; const valStr=document.getElementById('nc-val').value
  const val=parseFloat(valStr.replace(/\\./g,'').replace(',','.'))
  const st=getSelStatus()
  if(!cli||!tab||!val){ alert('Preencha cliente, tabela e valor'); return }
  await sb.from('vendas').insert({data:new Date().toISOString(),cliente:cli,cpf,telefone:telAtivo.replace(/\\D/g,''),produto:tab,tabela_banco:tab,valor:val,status:st,percentual_comissao:0,valor_comissao:0,comissao_empresa:0,valor_comissao_empresa:0,conferido:false,alterado_vendedor:false})
  await carregarVenda(telAtivo); await carregarConvs()
}
async function salvarEdicao() {
  const cli=document.getElementById('e-cli').value; const tab=document.getElementById('e-tab').value
  const valStr=document.getElementById('e-val').value; const val=parseFloat(valStr.replace(/\\./g,'').replace(',','.'))
  const st=getSelStatus(); const obs=document.getElementById('e-obs').value
  ...
  const ok=document.getElementById('sync-ok'); ok.style.display='block'
  setTimeout(async()=>{ editando=false; await carregarVenda(telAtivo); await carregarConvs(); renderProposta() },1200)
}

// ── Tabs ────────────────────────────────────────────────────────────
function showTab(id, tab) {
  document.querySelectorAll('.rp-tab').forEach(t=>t.classList.remove('active')); tab.classList.add('active')
