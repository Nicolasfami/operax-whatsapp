...
// CORRECAO: pairing code com delay e validacao de numero completo
...
app.get("/qr", (req, res) => {
  if (connected) return res.send(`<!DOCTYPE html><html><body style="background:#020c1e;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;font-family:sans-serif;"><div style="background:#0a1628;border:1px solid rgba(56,189,248,0.3);border-radius:20px;padding:40px;text-align:center;"><div style="font-size:60px;">checkmark</div><h2 style="color:#22c55e;">Conectado!</h2><p style="color:#94a3b8;">Pode fechar esta janela.</p><a href="/chat" style="display:inline-block;margin-top:16px;background:linear-gradient(135deg,#1d4ed8,#0ea5e9);color:#fff;padding:10px 24px;border-radius:10px;text-decoration:none;font-weight:700;">Abrir Chat</a></div></body></html>`)

  const html = `<!DOCTYPE html>
<html>
<head>
<title>OPERAX - Conectar</title>
...
.card{background:#0a1628;border:1px solid rgba(56,189,248,0.3);border-radius:20px;padding:36px;text-align:center;max-width:420px;width:100%}
.logo{font-size:28px;font-weight:900;color:#fff;letter-spacing:.08em;margin-bottom:4px}
.logo span{color:#38bdf8}
.sub{color:#64748b;font-size:13px;margin-bottom:24px}
.tabs{display:flex;gap:6px;margin-bottom:20px;background:#0f1f3d;border-radius:12px;padding:4px}
.tab{flex:1;padding:9px;border-radius:8px;border:none;cursor:pointer;font-size:13px;font-weight:700;background:transparent;color:#64748b}
.tab.active{background:linear-gradient(135deg,#1d4ed8,#0ea5e9);color:#fff}
#qr-s,#pair-s{display:none}
#qr-s.show,#pair-s.show{display:block}
.st{padding:9px 14px;border-radius:8px;font-size:12px;font-weight:700;margin:10px 0}
.st.wait{background:rgba(14,165,233,0.1);color:#38bdf8;border:1px solid rgba(14,165,233,0.3)}
.st.ok{background:rgba(34,197,94,0.1);color:#22c55e;border:1px solid rgba(34,197,94,0.3)}
.st.err{background:rgba(239,68,68,0.1);color:#ef4444;border:1px solid rgba(239,68,68,0.3)}
#qrcode{display:flex;justify-content:center;margin:14px 0;min-height:180px;align-items:center}
#qrcode img,#qrcode canvas{border-radius:10px;border:7px solid #fff}
input{width:100%;background:#0f1f3d;border:1.5px solid rgba(56,189,248,0.4);border-radius:10px;padding:11px 14px;color:#e2f4ff;font-size:15px;letter-spacing:.1em;outline:none;margin:6px 0 4px}
input:focus{border-color:#0ea5e9}
.hint{color:#64748b;font-size:11px;margin-bottom:14px}
button.btn{width:100%;background:linear-gradient(90deg,#1848cc,#0ea5e9);border:none;border-radius:10px;padding:13px;color:#fff;font-size:14px;font-weight:700;cursor:pointer}
.code-box{background:#0f1f3d;border:2px solid #38bdf8;border-radius:12px;padding:18px;margin:12px 0;letter-spacing:.4em;font-size:26px;font-weight:900;color:#38bdf8}
.note{color:#64748b;font-size:11px;text-align:left;line-height:1.7;margin-top:10px}
.note b{color:#94a3b8}
.spinner{border:3px solid rgba(56,189,248,0.2);border-top:3px solid #38bdf8;border-radius:50%;width:36px;height:36px;animation:spin 1s linear infinite;margin:20px auto}
@keyframes spin{to{transform:rotate(360deg)}}
.back-btn{width:100%;margin-top:10px;background:#0f1f3d;border:1px solid rgba(56,189,248,0.3);border-radius:10px;padding:10px;color:#7dd3fc;font-size:13px;font-weight:700;cursor:pointer}
</style>
</head>
<body>
<div class="card">
  <div class="logo">OPERAX <span>CHAT</span></div>
  <div class="sub">Vincular WhatsApp ao sistema</div>
  <div class="tabs">
    <button class="tab active" onclick="sw('qr')">QR Code</button>
    <button class="tab" onclick="sw('pair')">Codigo</button>
  </div>
  <div id="qr-s" class="show">
    <div class="st wait" id="qs">Aguardando QR Code...</div>
    <div id="qrcode"><div class="spinner"></div></div>
    <div class="note">Abra o <b>WhatsApp</b> no celular, va em <b>Dispositivos vinculados</b> e aponte a camera para o QR</div>
  </div>
  <div id="pair-s">
    <div id="pc">
      <div class="st wait">Digite o numero com codigo do pais (sem +)</div>
      <input type="tel" id="ph" placeholder="5511999999999" maxlength="15">
      <div class="hint">Exemplo: 55 + DDD + numero = 5511999999999</div>
      <button class="btn" onclick="reqCode()">Gerar codigo</button>
    </div>
    <div id="pr" style="display:none">
      <div class="st wait" id="ps">Gerando codigo, aguarde...</div>
      <div class="code-box" id="pb" style="display:none"></div>
      <div class="note" id="pi" style="display:none"><b>WhatsApp > Dispositivos vinculados > Vincular com numero de telefone</b> e digite o codigo acima antes de expirar</div>
      <button class="back-btn" onclick="document.getElementById('pc').style.display='block';document.getElementById('pr').style.display='none'">Voltar</button>
    </div>
  </div>
</div>
<script>
var s = io();
function sw(t) {
  document.querySelectorAll('.tab').forEach(function(x,i){ x.classList.toggle('active',(t==='qr'&&i===0)||(t==='pair'&&i===1)); });
  document.getElementById('qr-s').classList.toggle('show', t==='qr');
  document.getElementById('pair-s').classList.toggle('show', t==='pair');
}
s.on('qr', function(d) {
  document.getElementById('qs').textContent = 'Escaneie com o WhatsApp';
  document.getElementById('qrcode').innerHTML = '';
  new QRCode(document.getElementById('qrcode'), {text:d.qr, width:200, height:200, colorDark:'#000', colorLight:'#fff'});
});
s.on('connected', function() {
  ['qs','ps'].forEach(function(id){ var el=document.getElementById(id); if(el){el.className='st ok';el.textContent='Conectado! Redirecionando...';} });
  setTimeout(function(){ location.href='/chat'; }, 1500);
});
s.on('pairing_code', function(d) {
  var ps = document.getElementById('ps'); ps.className='st ok'; ps.textContent='Digite no WhatsApp:';
  var b = document.getElementById('pb'); b.textContent=d.code; b.style.display='block';
  document.getElementById('pi').style.display='block';
});
s.on('pairing_error', function(d) {
  var el = document.getElementById('ps'); el.className='st err'; el.textContent='Erro: '+d.error+' - tente novamente.';
});
function reqCode() {
  var p = document.getElementById('ph').value.replace(/\D/g,'');
  if(p.length < 12) { alert('Digite o numero completo com codigo do pais.\nExemplo: 5511999999999'); return; }
  document.getElementById('pc').style.display='none';
  document.getElementById('pr').style.display='block';
  document.getElementById('ps').className='st wait';
  document.getElementById('ps').textContent='Gerando codigo, aguarde...';
  ...
  fetch('/request-pairing', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({phone:p})})
    ...
    .then(function(d){
      ...
      else if(d.code){ document.getElementById('ps').className='st ok'; document.getElementById('ps').textContent='Digite no WhatsApp:'; var b=document.getElementById('pb'); b.textContent=d.code; b.style.display='block'; document.getElementById('pi').style.display='block'; }
    })
    .catch(function(){ document.getElementById('ps').className='st err'; document.getElementById('ps').textContent='Erro de conexao com o servidor.'; });
}
fetch('/status').then(function(r){ return r.json(); }).then(function(d){
  if(d.connected){ document.getElementById('qs').className='st ok'; document.getElementById('qs').textContent='Ja conectado!'; document.getElementById('qrcode').innerHTML='<div style="font-size:50px;margin:20px">ok</div>'; }
});
</script>
</body>
</html>`
...
<script>
var SUPA_URL = "https://ynxpowhzhnwqazdxshch.supabase.co";
var SUPA_KEY = "sb_publishable_aATPGJyG-Q8KuLLflByr8w_nrHxt0mt";
var sb = supabase.createClient(SUPA_URL, SUPA_KEY);
var socket = io();
var telAtivo = null, todasConvs = [], vendaAtiva = null, editando = false;
var STATUS_LIST = ['Pendente','Aguardando Pagamento','Aguardando Assinatura','Pago','Cancelado'];
var STATUS_CLS = {Pendente:'sp-pend','Aguardando Pagamento':'sp-agpag','Aguardando Assinatura':'sp-agass',Pago:'sp-pago',Cancelado:'sp-canc'};

function formatMoney(v) { try { return 'R$ ' + parseFloat(v).toLocaleString('pt-BR',{minimumFractionDigits:2}); } catch(e) { return 'R$ 0,00'; } }
...
async function checkStatus() {
  try {
    ...
    var b = document.getElementById('conn-badge'); var l = document.getElementById('qr-link');
    if(d.connected) { b.className='tb-badge badge-ok'; b.innerHTML='<span class="dot"></span> Conectado'; l.style.display='none'; }
    else { b.className='tb-badge badge-err'; b.innerHTML='<span class="dot"></span> Desconectado'; l.style.display='inline-block'; }
  } catch(e) {}
}
...
async function carregarConvs() {
  var res = await sb.from('whatsapp_conversas').select('*').order('ultima_atualizacao',{ascending:false});
  todasConvs = res.data || []; renderConvs(todasConvs);
}
function filtrarConvs() {
  var q = document.getElementById('search-input').value.toLowerCase();
  renderConvs(q ? todasConvs.filter(function(c){ return (c.nome||c.telefone||'').toLowerCase().includes(q); }) : todasConvs);
}
async function renderConvs(lista) {
  var el = document.getElementById('convs-list');
  if(!lista.length) { el.innerHTML = '<div style="padding:16px;text-align:center;color:rgba(148,185,210,.50);font-size:12px;">Nenhuma conversa</div>'; return; }
  var html = '';
  for(var i=0; i<lista.length; i++) {
    var c = lista[i]; var tel = c.telefone||''; var nome = c.nome||tel;
    var prev = (c.ultimo_msg||'').substring(0,28);
    ...
    var vd = await sb.from('vendas').select('status,valor').eq('telefone',tel.replace(/\D/g,'')).order('id',{ascending:false}).limit(1);
    var tag = '';
    if(vd.data && vd.data[0]) {
      var s = vd.data[0].status; var v = formatMoney(vd.data[0].valor);
      if(s==='Pago') tag='<span class="prop-tag pt-pago">Pago: '+v+'</span>';
      else if(s==='Aguardando Assinatura') tag='<span class="prop-tag pt-agass">Ag. Assin.</span>';
      else if(s==='Aguardando Pagamento') tag='<span class="prop-tag pt-agpag">Ag. Pgto</span>';
      else if(s) tag='<span class="prop-tag pt-pend">'+s+': '+v+'</span>';
    }
    var nomeSafe = nome.replace(/\\/g,'\\\\').replace(/'/g,"\\'");
    html += '<div class="conv '+ativo+'" onclick="selecionarConv(\''+tel+'\',\''+nomeSafe+'\')">';
    html += '<div class="av">'+ini+'</div>';
    html += '<div class="conv-info"><div class="conv-top"><span class="conv-name">'+nome+'</span>'+nr+'</div>';
    html += '<div class="conv-prev">'+prev+'</div>'+tag+'</div></div>';
  }
  el.innerHTML = html;
}
async function selecionarConv(tel, nome) {
  telAtivo = tel; editando = false;
  document.getElementById('ch-av').textContent = nome.split(' ').slice(0,2).map(function(p){ return p[0]?p[0].toUpperCase():''; }).join('');
  document.getElementById('ch-name').textContent = nome;
  document.getElementById('ch-status').textContent = tel;
  renderConvs(todasConvs);
  await sb.from('whatsapp_conversas').update({nao_lidas:0}).eq('telefone',tel);
  await carregarMsgs(tel); await carregarVenda(tel); renderRespostas();
}
async function carregarMsgs(tel) {
  var res = await sb.from('whatsapp_mensagens').select('*').eq('telefone',tel).order('timestamp').limit(100);
  var el = document.getElementById('msgs-area');
  var data = res.data || [];
  if(!data.length) { el.innerHTML = '<div class="empty-chat">Nenhuma mensagem ainda.</div>'; return; }
  var html = '';
  for(var i=0; i<data.length; i++) {
    var m = data[i]; var cls = m.de_mim?'msg-m':'msg-d'; var ts = (m.timestamp||'').substring(0,16);
    html += '<div class="'+cls+'">'+m.conteudo+'<div class="msg-ts">'+(m.de_mim?'ok ':'')+ts+'</div></div>';
  }
  el.innerHTML = html; el.scrollTop = el.scrollHeight;
}
async function carregarVenda(tel) {
  var t = tel.replace(/\D/g,'');
  var res = await sb.from('vendas').select('*').eq('telefone',t).order('id',{ascending:false}).limit(1);
  vendaAtiva = res.data && res.data[0] ? res.data[0] : null; renderProposta();
}
async function iniciarConv() {
  var t = document.getElementById('new-tel').value.replace(/\D/g,'');
  if(t.length<10) { alert('Numero invalido'); return; }
  await sb.from('whatsapp_conversas').upsert({telefone:t,ultimo_msg:'',ultima_atualizacao:new Date().toISOString(),nao_lidas:0},{onConflict:'telefone'});
  document.getElementById('new-tel').value=''; await carregarConvs(); selecionarConv(t,t);
}
async function enviar() {
  var txt = document.getElementById('msg-input').value.trim();
  if(!txt||!telAtivo) return;
  var r = await fetch('/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to:telAtivo,message:txt})});
  var d = await r.json();
  if(d.ok) {
    await sb.from('whatsapp_mensagens').insert({telefone:telAtivo,conteudo:txt,de_mim:true,timestamp:new Date().toISOString()});
    await sb.from('whatsapp_conversas').upsert({telefone:telAtivo,ultimo_msg:txt,ultima_atualizacao:new Date().toISOString(),nao_lidas:0},{onConflict:'telefone'});
    document.getElementById('msg-input').value=''; await carregarMsgs(telAtivo); await carregarConvs();
  } else alert('Erro ao enviar: '+(d.error||'desconhecido'));
}
async function renderRespostas() {
  var res = await sb.from('whatsapp_respostas_rapidas').select('*').order('id');
  var el = document.getElementById('tab-rr'); var html = '<div class="rp-sec">Respostas rapidas</div>';
  var data = res.data || [];
  for(var i=0; i<data.length; i++) {
    var rr = data[i];
    var textoSafe = rr.texto.replace(/\\/g,'\\\\').replace(/'/g,"\\'");
    html += '<div class="rr-item" onclick="usarResposta(\''+textoSafe+'\')">';
    html += '<div><div class="rr-title">'+rr.titulo+'</div><div class="rr-prev">'+rr.texto+'</div></div>';
    html += '<span class="rr-del" onclick="event.stopPropagation();deletarResposta('+rr.id+')">X</span></div>';
  }
  html += '<div class="add-rr" onclick="novaResposta()">+ Nova resposta</div>';
  el.innerHTML = html;
}
...
async function novaResposta() {
  var tit = prompt('Titulo:'); if(!tit) return;
  var txt = prompt('Texto:'); if(!txt) return;
  await sb.from('whatsapp_respostas_rapidas').insert({titulo:tit,texto:txt}); renderRespostas();
}
function renderProposta() {
  var el = document.getElementById('tab-prop');
  if(!telAtivo) { el.innerHTML='<div class="no-prop">Selecione uma conversa</div>'; return; }
  if(!editando) {
    if(vendaAtiva) {
      var v = vendaAtiva;
      var html = '<div class="prop-card">';
      html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;"><span class="pc-title">PROPOSTA ATIVA</span><span style="font-size:9px;color:#4ade80;">Supabase</span></div>';
      html += '<div class="pf-label">Cliente</div><div class="pf-value">'+(v.cliente||'--')+'</div>';
      html += '<div class="pf-label">Tabela/Banco</div><div class="pf-value">'+(v.tabela_banco||'--')+'</div>';
      html += '<div class="pf-label">Valor</div><div class="pf-valor">'+formatMoney(v.valor)+'</div>';
      html += '<div class="pf-label" style="margin-top:8px;">Status</div><div style="margin-top:3px;">'+pillHtml(v.status||'Pendente')+'</div>';
      if(v.observacao) html += '<div class="pf-label" style="margin-top:6px;">Observacao</div><div style="font-size:11px;color:#94a3b8;">'+v.observacao+'</div>';
      html += '<button class="edit-btn" onclick="abrirEdicao()">Editar proposta</button></div>';
      el.innerHTML = html;
    } else {
      var html = '<div class="no-prop">Nenhuma proposta ainda.</div><hr>';
      html += '<div class="rp-sec">Cadastrar venda</div>';
      html += '<div class="ef-label">Cliente</div><input class="ef-input" id="nc-cli" placeholder="Nome">';
      html += '<div class="ef-label">CPF</div><input class="ef-input" id="nc-cpf" placeholder="00000000000">';
      html += '<div class="ef-label">Tabela/Banco</div><input class="ef-input" id="nc-tab" placeholder="3RN CAPITAL...">';
      html += '<div class="ef-label">Valor R$</div><input class="ef-input" id="nc-val" placeholder="5.000,00">';
      html += '<div class="ef-label">Status</div><div class="st-grid">';
      var cls2 = ['pend','agpag','agass','pago','canc'];
      for(var i=0; i<STATUS_LIST.length; i++) {
        html += '<div class="st-chip sc-'+cls2[i]+' '+(i===0?'sel':'')+'" onclick="selSt(this)"><span class="st-dot"></span>'+STATUS_LIST[i]+'</div>';
      }
      html += '</div><button class="save-btn" onclick="salvarNovaVenda()">Salvar venda</button>';
      el.innerHTML = html;
    }
  } else {
    var v = vendaAtiva;
    var html = '<div class="rp-sec">Editar proposta</div>';
    html += '<div class="ef-label">Cliente</div><input class="ef-input" id="e-cli" value="'+(v.cliente||'')+'">';
    html += '<div class="ef-label">Tabela/Banco</div><input class="ef-input" id="e-tab" value="'+(v.tabela_banco||'')+'">';
    html += '<div class="ef-label">Valor R$</div><input class="ef-input" id="e-val" value="'+parseFloat(v.valor||0).toFixed(2).replace('.',',')+'">';
    html += '<div class="ef-label">Status</div><div class="st-grid">';
    var cls3 = ['pend','agpag','agass','pago','canc'];
    for(var i=0; i<STATUS_LIST.length; i++) {
      html += '<div class="st-chip sc-'+cls3[i]+' '+(STATUS_LIST[i]===v.status?'sel':'')+'" onclick="selSt(this)"><span class="st-dot"></span>'+STATUS_LIST[i]+'</div>';
    }
    html += '</div><div class="ef-label">Observacao</div><input class="ef-input" id="e-obs" value="'+(v.observacao||'')+'">';
    html += '<div id="sync-ok" class="sync-msg">Atualizado no Painel!</div>';
    html += '<button class="save-btn" onclick="salvarEdicao()">Salvar - Atualiza Painel</button>';
    html += '<div class="back-link" onclick="editando=false;renderProposta()">voltar</div>';
    el.innerHTML = html;
  }
}
...
function getSelStatus() {
  var s = document.querySelector('.st-chip.sel');
  if(!s) return 'Pendente';
  var cls = ['sc-pend','sc-agpag','sc-agass','sc-pago','sc-canc'];
  for(var i=0; i<cls.length; i++) { if(s.classList.contains(cls[i])) return STATUS_LIST[i]; }
  return 'Pendente';
}
async function salvarNovaVenda() {
  var cli = document.getElementById('nc-cli').value;
  var cpf = document.getElementById('nc-cpf').value.replace(/\D/g,'');
  var tab = document.getElementById('nc-tab').value;
  var valStr = document.getElementById('nc-val').value;
  var val = parseFloat(valStr.replace(/\./g,'').replace(',','.'));
  var st = getSelStatus();
  if(!cli||!tab||!val) { alert('Preencha cliente, tabela e valor'); return; }
  await sb.from('vendas').insert({data:new Date().toISOString(),cliente:cli,cpf:cpf,telefone:telAtivo.replace(/\D/g,''),produto:tab,tabela_banco:tab,valor:val,status:st,percentual_comissao:0,valor_comissao:0,comissao_empresa:0,valor_comissao_empresa:0,conferido:false,alterado_vendedor:false});
  await carregarVenda(telAtivo); await carregarConvs();
}
async function salvarEdicao() {
  var cli = document.getElementById('e-cli').value;
  var tab = document.getElementById('e-tab').value;
  var valStr = document.getElementById('e-val').value;
  var val = parseFloat(valStr.replace(/\./g,'').replace(',','.'));
  var st = getSelStatus();
  var obs = document.getElementById('e-obs').value;
  if(!val) { alert('Valor invalido'); return; }
  await sb.from('vendas').update({cliente:cli,tabela_banco:tab,produto:tab,valor:val,status:st,observacao:obs}).eq('id',vendaAtiva.id);
  var ok = document.getElementById('sync-ok'); ok.style.display='block';
  setTimeout(async function(){ editando=false; await carregarVenda(telAtivo); await carregarConvs(); renderProposta(); }, 1200);
}
function showTab(id, tab) {
  document.querySelectorAll('.rp-tab').forEach(function(t){ t.classList.remove('active'); }); tab.classList.add('active');
  document.getElementById('tab-rr').style.display = id==='rr'?'block':'none';
  document.getElementById('tab-prop').style.display = id==='prop'?'block':'none';
  if(id==='prop') renderProposta(); else renderRespostas();
}
