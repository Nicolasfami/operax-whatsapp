...
async function conectar() {
  ...
  sock = makeWASocket({ version, logger: pino({ level: "silent" }), printQRInTerminal: false, auth: state, browser: ["OPERAX", "Chrome", "1.0"] })
  sock.ev.on("connection.update", async ({ connection, lastDisconnect, qr }) => {
    if (qr) { qrCode = qr; qrReady = true; connected = false; io.emit("qr", { qr }) }
    if (connection === "close") {
      connected = false; qrReady = false; qrCode = null; pairingCode = null
    ...
    if (connection === "open") {
      connected = true; qrReady = false; qrCode = null; pairingCode = null
...
app.post("/request-pairing", async (req, res) => {
  ...
  try {
    ...
    if (!qrReady) return res.json({ error: "Tente novamente em 10 segundos" })
...
<div id="qr-s" class="show"><div class="st wait" id="qs">Aguardando QR Code...</div><div id="qrcode"><div class="spinner"></div></div><div class="note">Abra o <b>WhatsApp</b> no celular e escaneie o QR</div></div>
<div id="pair-s"><div id="pc"><div class="st wait">Digite seu numero com DDD</div><input type="tel" id="ph" placeholder="11999999999" maxlength="13"><button class="btn" onclick="reqCode()">Gerar codigo</button></div>
...
var s=io();
function sw(t){document.querySelectorAll('.tab').forEach(function(x,i){x.classList.toggle('active',(t==='qr'&&i===0)||(t==='pair'&&i===1));});document.getElementById('qr-s').classList.toggle('show',t==='qr');document.getElementById('pair-s').classList.toggle('show',t==='pair');}
s.on('qr',function(d){document.getElementById('qs').className='st ok';document.getElementById('qs').textContent='Escaneie com o WhatsApp';document.getElementById('qrcode').innerHTML='';new QRCode(document.getElementById('qrcode'),{text:d.qr,width:200,height:200,colorDark:'#000',colorLight:'#fff'});});
s.on('connected',function(){document.getElementById('qs').className='st ok';document.getElementById('qs').textContent='Conectado! Redirecionando...';setTimeout(function(){location.href='/chat';},1500);});
s.on('pairing_code',function(d){document.getElementById('ps').className='st ok';document.getElementById('ps').textContent='Digite no WhatsApp:';var b=document.getElementById('pb');b.textContent=d.code;b.style.display='block';document.getElementById('pi').style.display='block';});
s.on('pairing_error',function(d){document.getElementById('ps').className='st err';document.getElementById('ps').textContent='Erro: '+d.error;});
function reqCode(){var p=document.getElementById('ph').value.replace(/\D/g,'');if(p.length<10){alert('Numero invalido');return;}document.getElementById('pc').style.display='none';document.getElementById('pr').style.display='block';fetch('/request-pairing',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone:p})}).then(function(r){return r.json();}).then(function(d){if(d.error){document.getElementById('ps').className='st err';document.getElementById('ps').textContent='Erro: '+d.error;}else if(d.code){document.getElementById('ps').className='st ok';document.getElementById('ps').textContent='Digite no WhatsApp:';var b=document.getElementById('pb');b.textContent=d.code;b.style.display='block';document.getElementById('pi').style.display='block';}});}
fetch('/status').then(function(r){return r.json();}).then(function(d){if(d.connected){document.getElementById('qs').className='st ok';document.getElementById('qs').textContent='Ja conectado!';}});
</script></body></html>`)
})
...
.tb-logo{width:32px;height:32px;border-radius:9px;background:#1d4ed8;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:14px;flex-shrink:0;}
...
.conv{display:flex;align-items:center;gap:10px;padding:10px 14px;cursor:pointer;border-bottom:1px solid rgba(56,189,248,0.06);}
...
.nr{background:#38bdf8;color:#020c1e;font-size:9px;font-weight:800;border-radius:999px;padding:1px 6px;margin-left:4px;}
.sb-new{padding:10px 12px;border-top:1px solid rgba(56,189,248,0.10);}
.sb-new label{font-size:10px;font-weight:700;color:#38bdf8;text-transform:uppercase;display:block;margin-bottom:4px;}
...
.send-btn{width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,#1d4ed8,#0ea5e9);border:none;cursor:pointer;color:#fff;font-size:18px;}
...
.rp-tab{flex:1;padding:10px 4px;text-align:center;font-size:11px;font-weight:700;color:rgba(125,211,252,.50);cursor:pointer;border-bottom:2px solid transparent;}
...
.rr-prev{font-size:10px;color:rgba(148,185,210,.55);overflow:hidden;text-overflow:ellipsis;max-width:140px;white-space:nowrap;}
.rr-del{font-size:12px;color:rgba(248,113,133,.50);cursor:pointer;padding:2px 6px;}
...
.pf-label{font-size:9px;font-weight:700;color:rgba(125,211,252,.65);text-transform:uppercase;margin-bottom:2px;margin-top:6px;}
.pf-value{font-size:13px;font-weight:600;color:#e0f2fe;}
.pf-valor{font-size:16px;font-weight:800;color:#4ade80;}
.st-pill{display:inline-flex;align-items:center;gap:4px;padding:3px 9px;border-radius:99px;font-size:11px;font-weight:700;margin-top:4px;}
...
.ef-label{font-size:9px;font-weight:700;color:rgba(125,211,252,.65);text-transform:uppercase;margin:5px 0 2px;}
...
.empty-chat{flex:1;display:flex;align-items:center;justify-content:center;color:rgba(14,80,130,.55);font-size:13px;text-align:center;}
</style>
</head>
<body>
<div class="topbar">
...
<div class="layout">
  ...
  <div class="chat-main">
    ...
    <div class="msgs" id="msgs-area"><div class="empty-chat">Selecione uma conversa ao lado</div></div>
...
function pill(s) { return '<span class="st-pill '+(SC[s]||'sp-pend')+'">'+s+'</span>'; }
...
async function carregarConvs() {
  var r = await sb.from('whatsapp_conversas').select('*').order('ultima_atualizacao',{ascending:false});
  todasConvs = r.data || [];
...
function renderConvs(lista) {
  ...
  for (var i = 0; i < lista.length; i++) {
    ...
    var tel = c.telefone || '';
    var nome = c.nome || tel;
    var prev = (c.ultimo_msg||'').substring(0,30);
...
async function carregarMsgs(tel) {
  var r = await sb.from('whatsapp_mensagens').select('*').eq('telefone',tel).order('timestamp').limit(100);
  ...
  for (var i = 0; i < data.length; i++) {
    var m = data[i];
    var cls = m.de_mim ? 'msg-m' : 'msg-d';
    var ts = (m.timestamp||'').substring(0,16);
    html += '<div class="'+cls+'">'+m.conteudo+'<div class="msg-ts">'+ts+'</div></div>';
  }
  el.innerHTML = html;
...
async function carregarVenda(tel) {
  var r = await sb.from('vendas').select('*').eq('telefone',tel.replace(/\D/g,'')).order('id',{ascending:false}).limit(1);
  vendaAtiva = r.data && r.data[0] ? r.data[0] : null;
  renderProp();
}
...
async function enviar() {
  ...
  if (d.ok) {
    ...
    await carregarMsgs(telAtivo);
    await carregarConvs();
  } else { alert('Erro: '+(d.error||'?')); }
}

async function renderRR() {
  var r = await sb.from('whatsapp_respostas_rapidas').select('*').order('id');
  var el = document.getElementById('tab-rr');
  var html = '<div class="rp-sec">Respostas rapidas</div>';
  var data = r.data || [];
  for (var i = 0; i < data.length; i++) {
    var rr = data[i];
    html += '<div class="rr-item" onclick="useRR('+i+')">';
    html += '<div><div class="rr-title">'+rr.titulo+'</div><div class="rr-prev">'+rr.texto+'</div></div>';
    html += '<span class="rr-del" onclick="event.stopPropagation();delRR('+rr.id+')">X</span></div>';
  }
  html += '<div class="add-rr" onclick="novaRR()">+ Nova resposta</div>';
  el.innerHTML = html;
  window._rr = data;
}

function useRR(i) { if(window._rr && window._rr[i]) { document.getElementById('msg-input').value = window._rr[i].texto; document.getElementById('msg-input').focus(); } }
...
function renderProp() {
  ...
  if (!editando) {
    if (vendaAtiva) {
      ...
      html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;"><span style="font-size:11px;font-weight:700;color:#38bdf8;">PROPOSTA ATIVA</span><span style="font-size:9px;color:#4ade80;">sync</span></div>';
      ...
      html += '<div class="pf-label">Valor</div><div class="pf-valor">'+fm(v.valor)+'</div>';
      html += '<div class="pf-label">Status</div>'+pill(v.status||'Pendente');
      html += '<button class="edit-btn" onclick="abrirEd()">Editar proposta</button></div>';
      el.innerHTML = html;
    } else {
      var html = '<div class="no-prop">Nenhuma proposta ainda.</div>';
      html += '<hr style="border:none;border-top:1px solid rgba(56,189,248,.10);margin:8px 0;">';
      ...
      html += '<div class="ef-label">Tabela/Banco</div><input class="ef-input" id="nc-tab" placeholder="tabela...">';
      ...
      for (var i=0; i<SL.length; i++) html += '<div class="st-chip sc-'+cls2[i]+' '+(i===0?'sel':'')+'" onclick="selSt(this)"><span class="st-dot"></span>'+SL[i]+'</div>';
      html += '</div><button class="save-btn" onclick="salvarNova()">Salvar venda</button>';
      el.innerHTML = html;
    }
  } else {
    var v = vendaAtiva;
    ...
    for (var i=0; i<SL.length; i++) html += '<div class="st-chip sc-'+cls3[i]+' '+(SL[i]===v.status?'sel':'')+'" onclick="selSt(this)"><span class="st-dot"></span>'+SL[i]+'</div>';
    html += '</div><div id="sync-ok" class="sync-msg">Atualizado!</div>';
    html += '<button class="save-btn" onclick="salvarEd()">Salvar - Atualiza Painel</button>';
    html += '<div class="back-link" onclick="editando=false;renderProp()">voltar</div>';
    el.innerHTML = html;
  }
}
...
async function salvarNova() {
  ...
  var val=parseFloat(document.getElementById('nc-val').value.replace(/\./g,'').replace(',','.'));
  var st=getSt();
  if(!cli||!tab||!val){alert('Preencha tudo');return;}
  await sb.from('vendas').insert({data:new Date().toISOString(),cliente:cli,telefone:telAtivo.replace(/\D/g,''),produto:tab,tabela_banco:tab,valor:val,status:st,percentual_comissao:0,valor_comissao:0,comissao_empresa:0,valor_comissao_empresa:0,conferido:false,alterado_vendedor:false});
...
async function salvarEd() {
  ...
  var val=parseFloat(document.getElementById('e-val').value.replace(/\./g,'').replace(',','.'));
  var st=getSt();
  if(!val){alert('Valor invalido');return;}
  await sb.from('vendas').update({cliente:cli,tabela_banco:tab,produto:tab,valor:val,status:st}).eq('id',vendaAtiva.id);
  document.getElementById('sync-ok').style.display='block';
  setTimeout(async function(){ editando=false; await carregarVenda(telAtivo); renderProp(); }, 1200);
}
...
function showTab(id, tab) {
  ...
  if (id==='prop') renderProp(); else renderRR();
}
...
server.listen(PORT, function() { console.log("Servidor na porta " + PORT); conectar() })
